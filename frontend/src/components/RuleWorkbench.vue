...
const fields = [{"key":"meetingNo","label":"会议编号","type":"text"},{"key":"totalDirectors","label":"董事总数","type":"number"},{"key":"presentDirectors","label":"出席董事","type":"number"},{"key":"recused","label":"回避董事","type":"number"},{"key":"yesVotes","label":"赞成票","type":"number"},{"key":"noVotes","label":"反对票","type":"number"},{"key":"abstentions","label":"弃权票","type":"number"},{"key":"approvalThreshold","label":"通过门槛（%）","type":"number"}]
const form = reactive({"meetingNo":"BM-001","totalDirectors":7,"presentDirectors":6,"recused":1,"yesVotes":4,"noVotes":1,"abstentions":0,"approvalThreshold":66.67})
...
async function run() {
  busy.value = true
  error.value = ''
  try { result.value = await request('/api/enterprise/board/evaluate-vote', { method: 'POST', body: JSON.stringify(payload()) }) }
...
<template>
  <section class="rule-head">
    <div><span>DOMAIN CONTROL</span><h3>董事会表决有效性判断</h3><p>考虑回避董事、法定人数、有效票和通过门槛，形成决议结论。</p></div>
    <b>服务端强校验</b>
  </section>
